...
> **设计原则**: 不使用任何用户目录配置，避免配置分散和污染用户路径。
...
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│  层级3: 项目级配置 (Project config)                          │
│  - ./.analyzer.toml (隐藏文件，优先)                         │
│  - 或 ./analyzer.toml                                        │
│  - 最高优先级，项目特定设置                                  │
│  - 应提交到版本控制                                          │
└─────────────────────────────────────────────────────────────┘
...
1. **避免配置分散** - 用户目录配置容易被遗忘，导致"在我的机器上能运行"问题
2. **版本控制友好** - 所有配置都在项目内，便于团队协作
3. **可移植性** - 项目自包含，不依赖用户环境
4. **可预测性** - 配置位置明确，易于调试
